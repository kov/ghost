//! A real, throwaway `sshd` on loopback — the suite's Tier-1 high-fidelity remote.
//!
//! Shared fixture: `ssh_reboot.rs` drives ghost's transport straight at it, and
//! `shell_remote.rs` puts the whole shell on top of it, so the same reboot is
...
pub fn retry_some<T>(timeout: Duration, mut f: impl FnMut() -> Option<T>) -> Option<T> {
    let deadline = Instant::now() + timeout;
    loop {
        if let Some(v) = f() {
            return Some(v);
