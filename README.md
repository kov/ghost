...
- **Sessions & windows** — one window can drive several sessions at once; `Ctrl-Tab`
  ...
  return to (otherwise it drops to the fleet) — so exiting the shell in your only
  window, with nothing detached anywhere, quits ghost, just as closing that window
  by hand does.
