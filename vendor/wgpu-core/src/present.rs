...
impl Global {
    pub fn surface_get_current_texture(
        ...
        // ghost-local patch (report upstream): allocate the texture id only
        // once there is a texture to assign. Allocating before the acquire
        // leaked the id on every early exit — the `Err` returns and the
        // textureless `Ok` outputs (Timeout/Occluded/Outdated) — and each
        // leaked index later pads the textures registry map with a permanent
        // 24-byte `Vacant` element, so a render loop retrying an unacquirable
        // surface leaks without bound. See tests/surface_texture_id_leak.rs.
        ...
        let texture_id = output.texture.map(|texture| {
            let fid = self.hub.textures.prepare(texture_id_in);
            fid.assign(resource::Fallible::Valid(texture))
        });
